...
* Description: A BlackJack game with ASCII graphics.
* Monospace font and single line spacing recommended for optimal appearance.
* Date: 3/4/15
**/
...
// Card struct is a linked list node. This way, the number of cards a player has
...
// Hand struct is a linked list node. Each hand has the root node for a card
// linked list. This makes it possible to have multiple hands so a player can
// split their hand.
struct Hand {
	Card* root;
	Hand* next;
...
public:
	...
	// The current hand that the player is using is the last node in the hands
    // linked list.
	Hand* getCurrentHand() {
	...
	// Gets the hand before the current hand (i.e the new one if the current
    // hand is about to be deleted).
	Hand* getNewHand() {
		Hand* hand = this->hands;
	...
	string getMove(string message =
        "Enter your move! (hit, split, hold, or fold): ") {
		cout << message;
		string move = "";
		cin >> move;
		return (move == "hit" || move == "hold" || move == "fold")
            ? move
            : (move == "split")
                ? splittable(this->getCurrentHand())
                    ? move
                    : getMove("Invalid Move! Cannot split this hand! "
                        "(hit, hold, or fold) : ")
                : getMove("Invalid Move! (hit, split, hold, or fold): ");
	}

	// Adds a card to the current hand. Assumes card has already been removed
    // from a deck, and therefore has no child node.
	...
	// Splits the player's hand by removing the duplicate card from the current
    // hand and adding it to the next hand node in the linked list.
	void splitCurrentHand() {
		Hand* hand = this->getCurrentHand();
		if (!splittable(hand)) return;
		hand->next = new Hand();
		// Safe to assume that during a successful split, the card going into
        // the new hand is at the end of a linked list.
...
// Pops a random card from a deck. Just make sure to save this card and delete
// it later.
Card* dealCard(Hand* deck) {
	Card* card = deck->root;
	int c = 0;
...
// Prints out up to n cards in a hand. Leave segment number empty in calls.
// Also prints out some borders for the game table.
...
bool splittable(Hand* hand) {
	return (hand->root != NULL
        && hand->root->next != NULL
        && hand->root->faceVal == hand->root->next->faceVal
        && hand->root->next->next == NULL);
}

// Calculates the numerical value of a hand. Takes into account the dual values
// of aces.
int sumHand(Hand* hand) {
	Card* card = hand->root;
	int sum = 0;
...
// Gets the user's decision about whether or not to play another game after the
// termination of one.
string getDecision(string messagePrefix = "") {
	string decision = "";
...
void play() {
	...
	while (alive) {
		...
		// Either switches the player to their next hand or terminates the game
        // if he/she busts.
		if(playerSum > 21) {
		...
		// Terminates the game with victory if the dealer busts or the player
        // wins.
		else if (dealerSum > 21 || playerSum == 21) {
			won = 1;
			alive = false;
		}
		// If the game does not terminate (first case in if-else chain does not
        // always terminate game), queries the user for a decision.
		if (alive) {
			...
			if ((playerSum > dealerSum && playerSum < 21) || dealerSum < 17)
                dealer.addCard(dealCard(deck));
			// Terminates the game in a push if the user decides to hold when
            // the dealer refuses to draw more cards.
			else if (playerSum >= 17
                && playerSum == dealerSum && move == "hold") {
				won = 0;
				alive = false;
			}
		}
